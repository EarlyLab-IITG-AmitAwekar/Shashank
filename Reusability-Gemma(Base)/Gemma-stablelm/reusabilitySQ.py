...
STABLELM2_MODEL = 'stablelm2:1.6b'  # Changed from SMOLLM2_MODEL = 'smollm2:1.7b'
GEMMA3_RESULTS_CSV_PATH = '../gemma3_strategyqa_dataset_results.csv'
FINAL_RESULTS_CSV_PATH = 'gemma3_stablelm2_strategyqa_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'gemma3_stablelm2_strategyqa_accuracy_summary.csv'  # Updated filename
CHUNK_SAVE_SIZE = 20
...
def check_if_processing_needed(csv_path):
    """Check if StableLM2 processing is already complete"""  # Updated comment
    if not os.path.exists(csv_path):
        return True
    
    try:
        df = pd.read_csv(csv_path)
        stablelm2_cols = [  # Updated column names
        ...
        print(f"StableLM2 processing already complete. Found {len(df)} rows with StableLM2 data.")  # Updated message
        return False
    except Exception as e:
...
def main():
    """Load Gemma3 data, collect StableLM2 responses, combine and evaluate"""  # Updated description
    print("=== StableLM2 Processing and Evaluation Script (StrategyQA) ===")  # Updated title



    # Check if processing is needed
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        print("Skipping StableLM2 processing, moving directly to evaluation...")  # Updated message
    else:
        # Verify Gemma3 results exist
        if not os.path.exists(GEMMA3_RESULTS_CSV_PATH):
            print(f"Error: Gemma3 results file '{GEMMA3_RESULTS_CSV_PATH}' not found.")
            print("Please run the Gemma3 data collection script first.")
            return
        ...
        try:
            ...
            print(f"Loaded {len(df_gemma3)} rows from Gemma3 results")
        except Exception as e:
            print(f"Error loading Gemma3 results: {e}")
            return
        
        combined = []
        failed_requests = 0
        
        for idx, row in tqdm(df_gemma3.iterrows(), total=len(df_gemma3), desc="Processing StableLM2"):  # Updated description
            try:
                q = safe_get_column_value(row, 'question')
                cot_full = safe_get_column_value(row, 'gemma3_cot_steps_only')
                ...
                # StableLM2 responses with individual error handling
                responses = {}
                
                try:
                    responses['direct'] = generate_llm_response(STABLELM2_MODEL, PROMPT_DIRECT.format(question=q))  # Updated model
                except Exception as e:
                ...
                try:
                    responses['full'] = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_full))  # Updated model and prompt
                except Exception as e:
                ...
                try:
                    responses['corrupt'] = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_corrupt))  # Updated model and prompt
                except Exception as e:
                ...
                try:
                    responses['partial'] = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_partial))  # Updated model and prompt
                except Exception as e:
                ...
                combined.append({
                    ...
                    'gemma3_direct_raw': safe_get_column_value(row, 'gemma3_direct_raw'),
                    'gemma3_cot_raw': safe_get_column_value(row, 'gemma3_cot_raw'),
                    'gemma3_corrupted_cot_raw': safe_get_column_value(row, 'gemma3_corrupted_cot_raw'),
                    'gemma3_cot_steps_only': cot_full,
                    'corrupted_cot_steps_only': cot_corrupt,
                    'partial_cot_steps': cot_partial,
                    'gemma3_partial_cot_response': safe_get_column_value(row, 'gemma3_partial_cot_response'),
                    'stablelm2_direct_raw': responses['direct'],  # Updated column names
                    'stablelm2_with_cot_raw': responses['full'],
                    'stablelm2_with_corrupted_cot_raw': responses['corrupt'],
                    'stablelm2_with_partial_cot_raw': responses['partial'],
                })



                # Periodic save
                if (idx + 1) % CHUNK_SAVE_SIZE == 0 or (idx + 1) == len(df_gemma3):
                    try:
    ...
    try:
        ...
        df['gemma3_direct_answer'] = df['gemma3_direct_raw'].apply(extract_llm_boolean_answer)
        df['gemma3_cot_answer'] = df['gemma3_cot_raw'].apply(extract_llm_boolean_answer)
        df['gemma3_corrupted_cot_answer'] = df['gemma3_corrupted_cot_raw'].apply(extract_llm_boolean_answer)
        df['gemma3_partial_cot_answer'] = df['gemma3_partial_cot_response'].apply(extract_llm_boolean_answer)
        df['stablelm2_direct_answer'] = df['stablelm2_direct_raw'].apply(extract_llm_boolean_answer)  # Updated column names
        df['stablelm2_with_cot_answer'] = df['stablelm2_with_cot_raw'].apply(extract_llm_boolean_answer)
        df['stablelm2_with_corrupted_cot_answer'] = df['stablelm2_with_corrupted_cot_raw'].apply(extract_llm_boolean_answer)
        df['stablelm2_with_partial_cot_answer'] = df['stablelm2_with_partial_cot_raw'].apply(extract_llm_boolean_answer)
    except Exception as e:
    ...
    for i in range(min(3, len(df))):
        try:
            ...
            stablelm2_raw = str(df.iloc[i].get('stablelm2_direct_raw', ''))[:100]  # Updated variable name
            print(f"  StableLM2 Direct Raw: {stablelm2_raw}...")  # Updated label
            print(f"  StableLM2 Direct Answer: {df.iloc[i]['stablelm2_direct_answer']}")  # Updated label
            print()
        except Exception as e:
    ...
    settings = {
        "Gemma3 Direct vs. Original": 'gemma3_direct_answer',
        "Gemma3 CoT vs. Original": 'gemma3_cot_answer',
        "Gemma3 Corrupted CoT vs. Original": 'gemma3_corrupted_cot_answer',
        "Gemma3 Partial CoT vs. Original": 'gemma3_partial_cot_answer',
        "StableLM2 Direct vs. Original": 'stablelm2_direct_answer',  # Updated labels
        "StableLM2 with CoT vs. Original": 'stablelm2_with_cot_answer',
        "StableLM2 with Corrupted CoT vs. Original": 'stablelm2_with_corrupted_cot_answer',
        "StableLM2 with Partial CoT vs. Original": 'stablelm2_with_partial_cot_answer',
    }
