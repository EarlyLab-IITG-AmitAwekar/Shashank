...
df = pd.read_csv('gemma3_stablelm2_strategyqa_final_results.csv')
df['gemma3_answer'] = df['gemma3_cot_raw'].apply(extract_boolean_answer)
df['stablelm2_answer'] = df['stablelm2_with_cot_raw'].apply(extract_boolean_answer)

# Calculate match percentage
valid_df = df.dropna(subset=['gemma3_answer', 'stablelm2_answer'])
match_rate = (valid_df['gemma3_answer'] == valid_df['stablelm2_answer']).mean()
