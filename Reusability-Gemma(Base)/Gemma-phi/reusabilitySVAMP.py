...
PHI_MODEL = 'phi:2.7b'  # Changed from 'stablelm2:1.6b'
GEMMA_RESULTS_CSV_PATH = '../gemma3_svamp_results.csv'
FINAL_RESULTS_CSV_PATH = 'gemma_phi_svamp_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'gemma_phi_svamp_accuracy_summary.csv'  # Updated filename
...
def process_single_question(args):
    ...
    try:
        ...
        return idx, {
            'phi_direct_raw': resp_direct,  # Changed from 'stablelm_direct_raw'
            'phi_with_cot_raw': resp_full,  # Changed from 'stablelm_with_cot_raw'
            'phi_with_corrupted_cot_raw': resp_corrupt,  # Changed from 'stablelm_with_corrupted_cot_raw'
            'phi_with_partial_cot_raw': resp_partial,  # Changed from 'stablelm_with_partial_cot_raw'
        }
...
def main():
    """Load Gemma data, collect Phi responses, combine and evaluate"""
    print("=== Phi Processing and Evaluation Script (SVAMP Dataset) ===")  # Updated print message
    ...
    for batch_start in range(0, len(processed_data), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Processing Phi responses"):  # Updated description
                try:
                    idx, phi_results = future.result()  # Changed variable name
                    ...
                    combined.append({
                        ...
                        **phi_results  # Changed from stablelm_results
                    })
                except Exception as e:
    ...
    df['phi_direct_answer'] = df['phi_direct_raw'].apply(extract_llm_answer)  # Changed from stablelm
    df['phi_with_cot_answer'] = df['phi_with_cot_raw'].apply(extract_llm_answer)  # Changed from stablelm
    df['phi_with_corrupted_cot_answer'] = df['phi_with_corrupted_cot_raw'].apply(extract_llm_answer)  # Changed from stablelm
    df['phi_with_partial_cot_answer'] = df['phi_with_partial_cot_raw'].apply(extract_llm_answer)  # Changed from stablelm
    ...
    settings = {
        ...
        "Phi Direct vs. Original": 'phi_direct_answer',  # Changed from StableLM
        "Phi with CoT vs. Original": 'phi_with_cot_answer',  # Changed from StableLM
        "Phi with Corrupted CoT vs. Original": 'phi_with_corrupted_cot_answer',  # Changed from StableLM
        "Phi with Partial CoT vs. Original": 'phi_with_partial_cot_answer',  # Changed from StableLM
    }
