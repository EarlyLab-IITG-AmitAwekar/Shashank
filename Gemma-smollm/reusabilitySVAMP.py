...
SMOLLM_MODEL = 'smollm2:1.7b'
GEMMA_RESULTS_CSV_PATH = '../gemma3_svamp_results.csv'
FINAL_RESULTS_CSV_PATH = 'gemma_smollm_svamp_final_results.csv'
SUMMARY_CSV_PATH = 'gemma_smollm_svamp_accuracy_summary.csv'
...
def process_single_question(args):
    ...
    try:
        # Generate SmolLM responses
        resp_direct = generate_llm_response(SMOLLM_MODEL, PROMPT_DIRECT.format(question=q))
        resp_full = generate_llm_response(SMOLLM_MODEL, PROMPT_SMOLLM_WITH_COT.format(question=q, cot=cot_full))
        resp_corrupt = generate_llm_response(SMOLLM_MODEL, PROMPT_SMOLLM_WITH_COT.format(question=q, cot=cot_corrupt))
        resp_partial = generate_llm_response(SMOLLM_MODEL, PROMPT_SMOLLM_WITH_COT.format(question=q, cot=cot_partial))
        
        return idx, {
            'smollm_direct_raw': resp_direct,
            'smollm_with_cot_raw': resp_full,
            'smollm_with_corrupted_cot_raw': resp_corrupt,
            'smollm_with_partial_cot_raw': resp_partial,
        }
    except Exception as e:
        return idx, {
            'smollm_direct_raw': f"Error: {str(e)}",
            'smollm_with_cot_raw': f"Error: {str(e)}",
            'smollm_with_corrupted_cot_raw': f"Error: {str(e)}",
            'smollm_with_partial_cot_raw': f"Error: {str(e)}",
        }
...
def main():
    """Load Gemma data, collect SmolLM responses, combine and evaluate"""
    print("=== SmolLM Processing and Evaluation Script (SVAMP Dataset) ===")
    ...
    for batch_start in range(0, len(processed_data), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Processing SmolLM responses"):
                try:
                    idx, smollm_results = future.result()
    ...
    df['smollm_direct_answer'] = df['smollm_direct_raw'].apply(extract_llm_answer)
    df['smollm_with_cot_answer'] = df['smollm_with_cot_raw'].apply(extract_llm_answer)
    df['smollm_with_corrupted_cot_answer'] = df['smollm_with_corrupted_cot_raw'].apply(extract_llm_answer)
    df['smollm_with_partial_cot_answer'] = df['smollm_with_partial_cot_raw'].apply(extract_llm_answer)
    ...
    settings = {
        ...
        "SmolLM Direct vs. Original": 'smollm_direct_answer',
        "SmolLM with CoT vs. Original": 'smollm_with_cot_answer',
        "SmolLM with Corrupted CoT vs. Original": 'smollm_with_corrupted_cot_answer',
        "SmolLM with Partial CoT vs. Original": 'smollm_with_partial_cot_answer',
    }
