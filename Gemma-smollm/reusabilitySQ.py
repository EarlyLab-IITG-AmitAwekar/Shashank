...
SMOLLM2_MODEL = 'smollm2:1.7b'  # Changed from PHI_MODEL = 'phi:2.7b'
GEMMA3_RESULTS_CSV_PATH = '../gemma3_strategyqa_dataset_results.csv'
FINAL_RESULTS_CSV_PATH = 'gemma3_smollm2_strategyqa_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'gemma3_smollm2_strategyqa_accuracy_summary.csv'  # Updated filename
CHUNK_SAVE_SIZE = 20
...
def check_if_processing_needed(csv_path):
    """Check if SmolLM2 processing is already complete"""  # Updated comment
    if not os.path.exists(csv_path):
        return True
    
    try:
        df = pd.read_csv(csv_path)
        smollm2_cols = [  # Updated column names
            'smollm2_direct_raw',
        ...
        print(f"SmolLM2 processing already complete. Found {len(df)} rows with SmolLM2 data.")  # Updated message
        return False
    except Exception as e:
...
def main():
    """Load Gemma3 data, collect SmolLM2 responses, combine and evaluate"""  # Updated description
    print("=== SmolLM2 Processing and Evaluation Script (StrategyQA) ===")  # Updated title



    # Check if processing is needed
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        print("Skipping SmolLM2 processing, moving directly to evaluation...")  # Updated message
    else:
        ...
        for idx, row in tqdm(df_gemma3.iterrows(), total=len(df_gemma3), desc="Processing SmolLM2"):  # Updated description
            try:
                ...
                # SmolLM2 responses with individual error handling
                responses = {}
                
                try:
                    responses['direct'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_DIRECT.format(question=q))  # Updated model
                except Exception as e:
                ...
                try:
                    responses['full'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_full))  # Updated model and prompt
                except Exception as e:
                ...
                try:
                    responses['corrupt'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_corrupt))  # Updated model and prompt
                except Exception as e:
                ...
                try:
                    responses['partial'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_partial))  # Updated model and prompt
                except Exception as e:
                ...
                combined.append({
                    ...
                    'smollm2_direct_raw': responses['direct'],  # Updated column names
                    'smollm2_with_cot_raw': responses['full'],
                    'smollm2_with_corrupted_cot_raw': responses['corrupt'],
                    'smollm2_with_partial_cot_raw': responses['partial'],
                })
    ...
    try:
        ...
        df['smollm2_direct_answer'] = df['smollm2_direct_raw'].apply(extract_llm_boolean_answer)  # Updated column names
        df['smollm2_with_cot_answer'] = df['smollm2_with_cot_raw'].apply(extract_llm_boolean_answer)
        df['smollm2_with_corrupted_cot_answer'] = df['smollm2_with_corrupted_cot_raw'].apply(extract_llm_boolean_answer)
        df['smollm2_with_partial_cot_answer'] = df['smollm2_with_partial_cot_raw'].apply(extract_llm_boolean_answer)
    except Exception as e:
    ...
    for i in range(min(3, len(df))):
        try:
            ...
            smollm2_raw = str(df.iloc[i].get('smollm2_direct_raw', ''))[:100]  # Updated variable name
            print(f"  SmolLM2 Direct Raw: {smollm2_raw}...")  # Updated label
            print(f"  SmolLM2 Direct Answer: {df.iloc[i]['smollm2_direct_answer']}")  # Updated label
            print()
        except Exception as e:
