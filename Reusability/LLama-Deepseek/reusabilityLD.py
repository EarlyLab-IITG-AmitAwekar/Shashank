...
FINAL_RESULTS_CSV_PATH = 'llama_deepseek_gsm8k_final_results.csv'
SUMMARY_CSV_PATH = 'llama_deepseek_gsm8k_accuracy_summary.csv'
...
def main():
    """Load LLaMA data, collect DeepSeek responses, combine and evaluate"""
    print("=== DeepSeek Processing and Evaluation Script ===")
    ...
    # Collect DeepSeek responses and combine data
    combined_results = []
    
    print("\nStarting DeepSeek data collection and combination...")



    for idx, row in tqdm(df_llama.iterrows(), total=len(df_llama), desc="Processing DeepSeek Questions"):
        ...
        # DeepSeek responses using LLaMA's CoTs
        deepseek_direct_raw = generate_llm_response(DEEPSEEK_MODEL, PROMPT_DIRECT.format(question=question_text))
        deepseek_with_cot = generate_llm_response(DEEPSEEK_MODEL, PROMPT_DEEPSEEK_WITH_COT.format(question=question_text, cot=llama_cot_steps_only))
        deepseek_with_corrupted_cot = generate_llm_response(DEEPSEEK_MODEL, PROMPT_DEEPSEEK_WITH_COT.format(question=question_text, cot=corrupted_cot_steps_only))
        deepseek_with_partial_cot = generate_llm_response(DEEPSEEK_MODEL, PROMPT_DEEPSEEK_WITH_COT.format(question=question_text, cot=partial_cot_steps))



        # Combine LLaMA and DeepSeek data for this question
        combined_row = {
            # Original data
            ...
            # DeepSeek data
            'deepseek_direct_raw': deepseek_direct_raw,
            'deepseek_with_cot_raw': deepseek_with_cot,
            'deepseek_with_corrupted_cot_raw': deepseek_with_corrupted_cot,
            'deepseek_with_partial_cot_raw': deepseek_with_partial_cot,
        }
    ...
    print(f"\nDeepSeek data collection completed. Loading final results for evaluation...")
    ...
    df_combined['deepseek_direct_answer'] = df_combined['deepseek_direct_raw'].apply(extract_llm_answer)
    df_combined['deepseek_with_cot_answer'] = df_combined['deepseek_with_cot_raw'].apply(extract_llm_answer)
    df_combined['deepseek_with_corrupted_cot_answer'] = df_combined['deepseek_with_corrupted_cot_raw'].apply(extract_llm_answer)
    df_combined['deepseek_with_partial_cot_answer'] = df_combined['deepseek_with_partial_cot_raw'].apply(extract_llm_answer)
    ...
    df_combined['deepseek_direct_vs_original'] = df_combined['deepseek_direct_answer'] == df_combined['original_answer']
    df_combined['deepseek_w_cot_vs_original'] = df_combined['deepseek_with_cot_answer'] == df_combined['original_answer']
    df_combined['deepseek_w_corrupt_cot_vs_original'] = df_combined['deepseek_with_corrupted_cot_answer'] == df_combined['original_answer']
    df_combined['deepseek_w_partial_cot_vs_original'] = df_combined['deepseek_with_partial_cot_answer'] == df_combined['original_answer']
    ...
    model_accuracies = {
        ...
        "DeepSeek Direct vs. Original": df_combined['deepseek_direct_vs_original'].mean(),
        "DeepSeek with LLaMA's CoT vs. Original": df_combined['deepseek_w_cot_vs_original'].mean(),
        "DeepSeek with LLaMA's Corrupted CoT vs. Original": df_combined['deepseek_w_corrupt_cot_vs_original'].mean(),
        "DeepSeek with LLaMA's Partial CoT vs. Original": df_combined['deepseek_w_partial_cot_vs_original'].mean(),
    }
