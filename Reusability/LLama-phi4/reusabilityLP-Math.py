...
FINAL_RESULTS_CSV_PATH = 'llama_phi4_math_final_results.csv'
SUMMARY_CSV_PATH = 'llama_phi4_math_accuracy_summary.csv'
CHUNK_SAVE_SIZE = 20
...
def main():
    print("=== Phi4 Processing and Evaluation Script (MATH Dataset) ===")
    ...
    for idx, row in tqdm(df_llama.iterrows(), total=len(df_llama), desc="Processing"):
        try:
            ...
            # Generate Phi4 responses with error handling
            phi4_direct = generate_llm_response(PHI4_MODEL, PROMPT_DIRECT.format(question=q))
            phi4_with_cot = generate_llm_response(PHI4_MODEL, PROMPT_PHI4_WITH_COT.format(question=q, cot=cot))
            phi4_with_corr = generate_llm_response(PHI4_MODEL, PROMPT_PHI4_WITH_COT.format(question=q, cot=corrupted))
            phi4_with_part = generate_llm_response(PHI4_MODEL, PROMPT_PHI4_WITH_COT.format(question=q, cot=partial))
    ...
    try:
        ...
        answer_columns = {
            ...
            'phi4_direct_raw': 'phi4_direct_ans',
            'phi4_with_cot_raw': 'phi4_with_cot_ans',
            'phi4_with_corrupted_cot_raw': 'phi4_with_corrupted_cot_ans',
            'phi4_with_partial_cot_raw': 'phi4_with_partial_cot_ans'
        }
        ...
        comparisons = {
            ...
            'phi4_direct_vs_original': 'phi4_direct_ans',
            'phi4_w_cot_vs_original': 'phi4_with_cot_ans',
            'phi4_w_corrupt_cot_vs_original': 'phi4_with_corrupted_cot_ans',
            'phi4_w_partial_cot_vs_original': 'phi4_with_partial_cot_ans'
        }
        ...
        results = {
            ...
            "Phi4 Direct vs. Original": df_combined.get('phi4_direct_vs_original', pd.Series()).mean(),
            "Phi4 w/ CoT vs. Original": df_combined.get('phi4_w_cot_vs_original', pd.Series()).mean(),
            "Phi4 w/ Corrupted CoT vs. Original": df_combined.get('phi4_w_corrupt_cot_vs_original', pd.Series()).mean(),
            "Phi4 w/ Partial CoT vs. Original": df_combined.get('phi4_w_partial_cot_vs_original', pd.Series()).mean()
        }
