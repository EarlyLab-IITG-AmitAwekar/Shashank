...
FINAL_RESULTS_CSV_PATH = 'llama_gemma3_gsm8k_final_results.csv'
SUMMARY_CSV_PATH = 'llama_gemma3_gsm8k_accuracy_summary.csv'
...
def main():
    """Load LLaMA data, collect Gemma3 responses, combine and evaluate"""
    print("=== Gemma3 Processing and Evaluation Script ===")
    ...
    # Collect Gemma3 responses and combine data
    combined_results = []
    
    print("\nStarting Gemma3 data collection and combination...")


    for idx, row in tqdm(df_llama.iterrows(), total=len(df_llama), desc="Processing Gemma3 Questions"):
        ...
        # Gemma3 responses using LLaMA's CoTs
        gemma3_direct_raw = generate_llm_response(GEMMA3_MODEL, PROMPT_DIRECT.format(question=question_text))
        gemma3_with_cot = generate_llm_response(GEMMA3_MODEL, PROMPT_GEMMA3_WITH_COT.format(question=question_text, cot=llama_cot_steps_only))
        gemma3_with_corrupted_cot = generate_llm_response(GEMMA3_MODEL, PROMPT_GEMMA3_WITH_COT.format(question=question_text, cot=corrupted_cot_steps_only))
        gemma3_with_partial_cot = generate_llm_response(GEMMA3_MODEL, PROMPT_GEMMA3_WITH_COT.format(question=question_text, cot=partial_cot_steps))


        # Combine LLaMA and Gemma3 data for this question
        combined_row = {
            # Original data
            ...
            # Gemma3 data
            'gemma3_direct_raw': gemma3_direct_raw,
            'gemma3_with_cot_raw': gemma3_with_cot,
            'gemma3_with_corrupted_cot_raw': gemma3_with_corrupted_cot,
            'gemma3_with_partial_cot_raw': gemma3_with_partial_cot,
        }
    ...
    print(f"\nGemma3 data collection completed. Loading final results for evaluation...")
    ...
    df_combined['gemma3_direct_answer'] = df_combined['gemma3_direct_raw'].apply(extract_llm_answer)
    df_combined['gemma3_with_cot_answer'] = df_combined['gemma3_with_cot_raw'].apply(extract_llm_answer)
    df_combined['gemma3_with_corrupted_cot_answer'] = df_combined['gemma3_with_corrupted_cot_raw'].apply(extract_llm_answer)
    df_combined['gemma3_with_partial_cot_answer'] = df_combined['gemma3_with_partial_cot_raw'].apply(extract_llm_answer)
    ...
    df_combined['gemma3_direct_vs_original'] = df_combined['gemma3_direct_answer'] == df_combined['original_answer']
    df_combined['gemma3_w_cot_vs_original'] = df_combined['gemma3_with_cot_answer'] == df_combined['original_answer']
    df_combined['gemma3_w_corrupt_cot_vs_original'] = df_combined['gemma3_with_corrupted_cot_answer'] == df_combined['original_answer']
    df_combined['gemma3_w_partial_cot_vs_original'] = df_combined['gemma3_with_partial_cot_answer'] == df_combined['original_answer']
    ...
    model_accuracies = {
        ...
        "Gemma3 Direct vs. Original": df_combined['gemma3_direct_vs_original'].mean(),
        "Gemma3 with LLaMA's CoT vs. Original": df_combined['gemma3_w_cot_vs_original'].mean(),
        "Gemma3 with LLaMA's Corrupted CoT vs. Original": df_combined['gemma3_w_corrupt_cot_vs_original'].mean(),
        "Gemma3 with LLaMA's Partial CoT vs. Original": df_combined['gemma3_w_partial_cot_vs_original'].mean(),
    }
