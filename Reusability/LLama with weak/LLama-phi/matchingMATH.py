...
df = pd.read_csv('./llama_phi_math_final_results.csv')
...
df['phi_answer'] = df['phi4_with_cot_raw'].apply(lambda x: extract_clean_text(parse_json_answer(x)))

# Calculate match percentage
valid_df = df[(df['llama_answer'] != "") & (df['phi_answer'] != "")]
match_rate = (valid_df['llama_answer'].str.lower() == valid_df['phi_answer'].str.lower()).mean()
