...
FINAL_RESULTS_CSV_PATH = 'llama_phi_math_final_results.csv'
SUMMARY_CSV_PATH = 'llama_phi_math_accuracy_summary.csv'
CHUNK_SAVE_SIZE = 20
...
PROMPT_PHI4_WITH_COT = """You must ONLY use the provided reasoning below to answer the question. 
...
def remove_final_answer_from_cot(cot_text):
    ...
    to ensure only reasoning steps are passed to Phi4
    """
...
def process_single_row(args):
    ...
    idx, row_data, phi4_model, prompt_direct, prompt_cot = args
    
    try:
        q, cot_steps_only, corrupted_steps_only, partial_steps_only = row_data
        
        # Generate Phi4 responses
        phi4_direct = generate_llm_response(phi4_model, prompt_direct.format(question=q))
        phi4_with_cot = generate_llm_response(phi4_model, prompt_cot.format(question=q, cot=cot_steps_only))
        phi4_with_corr = generate_llm_response(phi4_model, prompt_cot.format(question=q, cot=corrupted_steps_only))
        phi4_with_part = generate_llm_response(phi4_model, prompt_cot.format(question=q, cot=partial_steps_only))
        
        return idx, {
            'phi4_direct_raw': phi4_direct,
            'phi4_with_cot_raw': phi4_with_cot,
            'phi4_with_corrupted_cot_raw': phi4_with_corr,
            'phi4_with_partial_cot_raw': phi4_with_part
        }
    except Exception as e:
        return idx, {
            'phi4_direct_raw': f"Error: {e}",
            'phi4_with_cot_raw': f"Error: {e}",
            'phi4_with_corrupted_cot_raw': f"Error: {e}",
            'phi4_with_partial_cot_raw': f"Error: {e}"
        }
...
def parse_json_answer(raw: str):
    ...
    # Strategy 4: Look for "Answer:" pattern (common in Phi4 responses)
    answer_patterns = [
        r'Answer:\s*(.+?)(?:\n|$)',
        r'The answer is:\s*(.+?)(?:\n|$)',
...
def check_if_processing_needed(csv_path):
    """Check if Phi4 processing is already complete"""
    ...
    try:
        ...
        phi4_cols = [
            'phi4_direct_raw',
            'phi4_with_cot_raw', 
            'phi4_with_corrupted_cot_raw',
            'phi4_with_partial_cot_raw'
        ]
        
        # Check if all Phi4 columns exist and have data
        ...
        print(f"Phi4 processing already complete. Found {len(df)} rows with Phi4 data.")
        return False
    except Exception as e:
...
def main():
    print("=== Enhanced Phi4 Processing with Parallel Execution ===")

    # Check if processing is needed
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        print("Skipping Phi4 processing, moving directly to evaluation...")
    else:
        # Check CSV existence
        ...
        for chunk_start in range(0, len(processed_data), chunk_size):
            ...
            for item in chunk_data:
                ...
                args_list.append((item['idx'], row_data, PHI4_MODEL, PROMPT_DIRECT, PROMPT_PHI4_WITH_COT))
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                results = list(tqdm(
                    ...
                    total=len(args_list),
                    desc=f"Processing Phi4 responses"
                ))
            
            # Combine results with original data
            for (idx, phi4_results), item in zip(results, chunk_data):
                row = item['row']
                combined.append({
    ...
    try:
        ...
        # Extract Phi4 answers using enhanced JSON/pattern parsing
        print("Parsing Phi4 answers with enhanced extraction...")
        df_combined['phi4_direct_text'] = df_combined['phi4_direct_raw'].apply(parse_json_answer)
        df_combined['phi4_with_cot_text'] = df_combined['phi4_with_cot_raw'].apply(parse_json_answer)
        df_combined['phi4_with_corrupted_cot_text'] = df_combined['phi4_with_corrupted_cot_raw'].apply(parse_json_answer)
        df_combined['phi4_with_partial_cot_text'] = df_combined['phi4_with_partial_cot_raw'].apply(parse_json_answer)
        ...
        # Phi4 comparisons
        print("Computing Phi4 similarities...")
        phi4_direct_sim = compute_similarity_optimized(reference_answers, df_combined['phi4_direct_text'].tolist())
        phi4_cot_sim = compute_similarity_optimized(reference_answers, df_combined['phi4_with_cot_text'].tolist())
        phi4_corrupted_sim = compute_similarity_optimized(reference_answers, df_combined['phi4_with_corrupted_cot_text'].tolist())
        phi4_partial_sim = compute_similarity_optimized(reference_answers, df_combined['phi4_with_partial_cot_text'].tolist())
        ...
        df_combined['phi4_direct_similarity'] = phi4_direct_sim
        df_combined['phi4_cot_similarity'] = phi4_cot_sim
        df_combined['phi4_corrupted_cot_similarity'] = phi4_corrupted_sim
        df_combined['phi4_partial_cot_similarity'] = phi4_partial_sim
        ...
        results = {
            ...
            "Phi4 Direct vs. Original": np.mean(phi4_direct_sim),
            "Phi4 w/ CoT vs. Original": np.mean(phi4_cot_sim),
            "Phi4 w/ Corrupted CoT vs. Original": np.mean(phi4_corrupted_sim),
            "Phi4 w/ Partial CoT vs. Original": np.mean(phi4_partial_sim)
        }

        # Display results
        print("\n" + "="*60)
        print("FINAL RESULTS (Optimized Processing)")
        print("="*60)
