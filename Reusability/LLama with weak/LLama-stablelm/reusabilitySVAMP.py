...
FINAL_RESULTS_CSV_PATH = 'llama_stablelm2_svamp_final_results.csv'
SUMMARY_CSV_PATH = 'llama_stablelm2_svamp_accuracy_summary.csv'
...
def process_single_question(args):
    ...
    try:
        ...
        resp_direct = generate_llm_response(STABLELM2_MODEL, PROMPT_DIRECT.format(question=q))
        resp_full = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_full))
        resp_corrupt = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_corrupt))
        resp_partial = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_partial))
        ...
        return idx, {
            'stablelm2_direct_raw': f"Error: {str(e)}",
            'stablelm2_with_cot_raw': f"Error: {str(e)}",
            'stablelm2_with_corrupted_cot_raw': f"Error: {str(e)}",
            'stablelm2_with_partial_cot_raw': f"Error: {str(e)}",
        }
...
def main():
    ...
    print("=== StableLM2 Processing and Evaluation Script (SVAMP Dataset) ===")
    ...
    for batch_start in range(0, len(processed_data), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Processing StableLM2 responses"):
                try:
                    idx, stablelm2_results = future.result()
