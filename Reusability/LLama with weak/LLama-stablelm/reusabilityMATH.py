...
STABLELM2_MODEL = 'stablelm2:1.6b'
LLAMA_RESULTS_CSV_PATH = '../../llama_math_dataset_results.csv'
FINAL_RESULTS_CSV_PATH = 'llama_stablelm2_math_final_results.csv'
SUMMARY_CSV_PATH = 'llama_stablelm2_math_accuracy_summary.csv'
CHUNK_SAVE_SIZE = 20
...
PROMPT_STABLELM2_WITH_COT = """You must ONLY use the provided reasoning below to answer the question. 
...
def remove_final_answer_from_cot(cot_text):
    ...
    to ensure only reasoning steps are passed to StableLM2
    """
...
def process_single_row(args):
    ...
    idx, row_data, stablelm2_model, prompt_direct, prompt_cot = args
    
    try:
        q, cot_steps_only, corrupted_steps_only, partial_steps_only = row_data
        
        # Generate StableLM2 responses
        stablelm2_direct = generate_llm_response(stablelm2_model, prompt_direct.format(question=q))
        stablelm2_with_cot = generate_llm_response(stablelm2_model, prompt_cot.format(question=q, cot=cot_steps_only))
        stablelm2_with_corr = generate_llm_response(stablelm2_model, prompt_cot.format(question=q, cot=corrupted_steps_only))
        stablelm2_with_part = generate_llm_response(stablelm2_model, prompt_cot.format(question=q, cot=partial_steps_only))
        
        return idx, {
            'stablelm2_direct_raw': stablelm2_direct,
            'stablelm2_with_cot_raw': stablelm2_with_cot,
            'stablelm2_with_corrupted_cot_raw': stablelm2_with_corr,
            'stablelm2_with_partial_cot_raw': stablelm2_with_part
        }
    except Exception as e:
        return idx, {
            'stablelm2_direct_raw': f"Error: {e}",
            'stablelm2_with_cot_raw': f"Error: {e}",
            'stablelm2_with_corrupted_cot_raw': f"Error: {e}",
            'stablelm2_with_partial_cot_raw': f"Error: {e}"
        }
...
def parse_json_answer(raw: str):
    ...
    # Strategy 4: Look for "Answer:" pattern (common in StableLM2 responses)
    answer_patterns = [
        r'Answer:\s*(.+?)(?:\n|$)',
        r'The answer is:\s*(.+?)(?:\n|$)',
...
def check_if_processing_needed(csv_path):
    """Check if StableLM2 processing is already complete"""
    ...
    try:
        ...
        stablelm2_cols = [
            'stablelm2_direct_raw',
            'stablelm2_with_cot_raw', 
            'stablelm2_with_corrupted_cot_raw',
            'stablelm2_with_partial_cot_raw'
        ]
        
        # Check if all StableLM2 columns exist and have data
        for col in stablelm2_cols:
            if col not in df.columns:
                return True
            if df[col].isna().all():
                return True
        
        print(f"StableLM2 processing already complete. Found {len(df)} rows with StableLM2 data.")
        return False
    except Exception as e:
...
def main():
    print("=== Enhanced StableLM2 Processing with Parallel Execution ===")

    # Check if processing is needed
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        print("Skipping StableLM2 processing, moving directly to evaluation...")
    else:
        # Check CSV existence
        ...
        for idx, row in df_llama.iterrows():
            ...
            cot_raw = safe_get_value(row, 'llama_cot_steps_only')
        ...
        for chunk_start in range(0, len(processed_data), chunk_size):
            ...
            for item in chunk_data:
                ...
                args_list.append((item['idx'], row_data, STABLELM2_MODEL, PROMPT_DIRECT, PROMPT_STABLELM2_WITH_COT))
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                results = list(tqdm(
                    ...
                    total=len(args_list),
                    desc=f"Processing StableLM2 responses"
                ))
            
            # Combine results with original data
            for (idx, stablelm2_results), item in zip(results, chunk_data):
                row = item['row']
                combined.append({
                    ...
                    'llama_cot_steps_only': item['cot_steps_only'],
    ...
    try:
        ...
        # Extract LLaMA answers
        llama_direct_boxed = df_combined['llama_direct_raw'].apply(lambda x: find_boxed_content(str(x)))
        df_combined['llama_direct_text'] = llama_direct_boxed.apply(lambda x: x[-1] if x else "")
        
        llama_cot_boxed = df_combined['llama_cot_raw'].apply(lambda x: find_boxed_content(str(x)))
        df_combined['llama_cot_text'] = llama_cot_boxed.apply(lambda x: x[-1] if x else "")
        
        llama_corrupted_boxed = df_combined['llama_corrupted_cot_raw'].apply(lambda x: find_boxed_content(str(x)))
        df_combined['llama_corrupted_cot_text'] = llama_corrupted_boxed.apply(lambda x: x[-1] if x else "")
        
        llama_partial_boxed = df_combined['llama_partial_cot_response'].apply(lambda x: find_boxed_content(str(x)))
        df_combined['llama_partial_cot_text'] = llama_partial_boxed.apply(lambda x: x[-1] if x else "")
        
        # Extract StableLM2 answers using enhanced JSON/pattern parsing
        print("Parsing StableLM2 answers with enhanced extraction...")
        df_combined['stablelm2_direct_text'] = df_combined['stablelm2_direct_raw'].apply(parse_json_answer)
        df_combined['stablelm2_with_cot_text'] = df_combined['stablelm2_with_cot_raw'].apply(parse_json_answer)
        df_combined['stablelm2_with_corrupted_cot_text'] = df_combined['stablelm2_with_corrupted_cot_raw'].apply(parse_json_answer)
        df_combined['stablelm2_with_partial_cot_text'] = df_combined['stablelm2_with_partial_cot_raw'].apply(parse_json_answer)
        ...
        # LLaMA comparisons
        print("Computing LLaMA similarities...")
        llama_direct_sim = compute_similarity_optimized(reference_answers, df_combined['llama_direct_text'].tolist())
        llama_cot_sim = compute_similarity_optimized(reference_answers, df_combined['llama_cot_text'].tolist())
        llama_corrupted_sim = compute_similarity_optimized(reference_answers, df_combined['llama_corrupted_cot_text'].tolist())
        llama_partial_sim = compute_similarity_optimized(reference_answers, df_combined['llama_partial_cot_text'].tolist())
        
        # StableLM2 comparisons
        print("Computing StableLM2 similarities...")
        stablelm2_direct_sim = compute_similarity_optimized(reference_answers, df_combined['stablelm2_direct_text'].tolist())
        stablelm2_cot_sim = compute_similarity_optimized(reference_answers, df_combined['stablelm2_with_cot_text'].tolist())
        stablelm2_corrupted_sim = compute_similarity_optimized(reference_answers, df_combined['stablelm2_with_corrupted_cot_text'].tolist())
        stablelm2_partial_sim = compute_similarity_optimized(reference_answers, df_combined['stablelm2_with_partial_cot_text'].tolist())

        # Add similarity scores to dataframe
        df_combined['llama_direct_similarity'] = llama_direct_sim
        df_combined['llama_cot_similarity'] = llama_cot_sim
        df_combined['llama_corrupted_cot_similarity'] = llama_corrupted_sim
        df_combined['llama_partial_cot_similarity'] = llama_partial_sim
        df_combined['stablelm2_direct_similarity'] = stablelm2_direct_sim
        df_combined['stablelm2_cot_similarity'] = stablelm2_cot_sim
        df_combined['stablelm2_corrupted_cot_similarity'] = stablelm2_corrupted_sim
        df_combined['stablelm2_partial_cot_similarity'] = stablelm2_partial_sim

        # Calculate average similarities
        results = {
            "LLaMA Direct vs. Original": np.mean(llama_direct_sim),
            "LLaMA CoT vs. Original": np.mean(llama_cot_sim),
            "LLaMA Corrupted CoT vs. Original": np.mean(llama_corrupted_sim),
            "LLaMA Partial CoT vs. Original": np.mean(llama_partial_sim),
            "StableLM2 Direct vs. Original": np.mean(stablelm2_direct_sim),
            "StableLM2 w/ CoT vs. Original": np.mean(stablelm2_cot_sim),
            "StableLM2 w/ Corrupted CoT vs. Original": np.mean(stablelm2_corrupted_sim),
            "StableLM2 w/ Partial CoT vs. Original": np.mean(stablelm2_partial_sim)
        }

        # Display results
        print("\n" + "="*60)
        print("FINAL RESULTS (StableLM2 Optimized Processing)")
        print("="*60)
