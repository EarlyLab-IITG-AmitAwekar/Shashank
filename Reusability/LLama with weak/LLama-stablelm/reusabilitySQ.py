...
FINAL_RESULTS_CSV_PATH = 'llama_stablelm2_strategyqa_final_results.csv'
SUMMARY_CSV_PATH = 'llama_stablelm2_strategyqa_accuracy_summary.csv'
CHUNK_SAVE_SIZE = 20
...
def main():
    ...
    print("=== StableLM2 Processing and Evaluation Script (StrategyQA) ===")
    ...
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        ...
        for idx, row in tqdm(df_llama.iterrows(), total=len(df_llama), desc="Processing StableLM2"):
            try:
                ...
                try:
                    responses['direct'] = generate_llm_response(STABLELM2_MODEL, PROMPT_DIRECT.format(question=q))
                except Exception as e:
                ...
                try:
                    responses['full'] = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_full))
                except Exception as e:
                ...
                try:
                    responses['corrupt'] = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_corrupt))
                except Exception as e:
                ...
                try:
                    responses['partial'] = generate_llm_response(STABLELM2_MODEL, PROMPT_STABLELM2_WITH_COT.format(question=q, cot=cot_partial))
                except Exception as e:
                ...
                combined.append({
                    ...
                    'stablelm2_direct_raw': responses['direct'],
    ...
    try:
        ...
        df['stablelm2_direct_answer'] = df['stablelm2_direct_raw'].apply(extract_llm_boolean_answer)
    ...
    for i in range(min(3, len(df))):
        try:
            ...
            stablelm2_raw = str(df.iloc[i].get('stablelm2_direct_raw', ''))[:100]
            print(f"  StableLM2 Direct Raw: {stablelm2_raw}...")
