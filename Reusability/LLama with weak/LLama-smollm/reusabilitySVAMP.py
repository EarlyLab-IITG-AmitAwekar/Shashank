...
FINAL_RESULTS_CSV_PATH = 'llama_smollm2_svamp_final_results.csv'
SUMMARY_CSV_PATH = 'llama_smollm2_svamp_accuracy_summary.csv'
...
def process_single_question(args):
    ...
    try:
        ...
        resp_direct = generate_llm_response(SMOLLM2_MODEL, PROMPT_DIRECT.format(question=q))
        resp_full = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_full))
        resp_corrupt = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_corrupt))
        resp_partial = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_partial))
        
        return idx, {
            'smollm2_direct_raw': resp_direct,
        ...
        return idx, {
            'smollm2_direct_raw': f"Error: {str(e)}",
            'smollm2_with_cot_raw': f"Error: {str(e)}",
            'smollm2_with_corrupted_cot_raw': f"Error: {str(e)}",
            'smollm2_with_partial_cot_raw': f"Error: {str(e)}",
        }
...
def main():
    ...
    print("=== SmolLM2 Processing and Evaluation Script (SVAMP Dataset) ===")
    ...
    for batch_start in range(0, len(processed_data), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Processing SmolLM2 responses"):
                try:
                    idx, smollm2_results = future.result()
    ...
    df['smollm2_direct_answer'] = df['smollm2_direct_raw'].apply(extract_llm_answer)
    ...
    settings = {
        ...
        "SmolLM2 Direct vs. Original": 'smollm2_direct_answer',
