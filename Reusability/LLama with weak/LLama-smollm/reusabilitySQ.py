...
SMOLLM2_MODEL = 'smollm2:1.7b'  # Changed from STABLELM2_MODEL = 'stablelm2:1.6b'
LLAMA_RESULTS_CSV_PATH = '../../llama_strategyqa_dataset_results.csv'
FINAL_RESULTS_CSV_PATH = 'llama_smollm2_strategyqa_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'llama_smollm2_strategyqa_accuracy_summary.csv'  # Updated filename
CHUNK_SAVE_SIZE = 20
...
def main():
    """Load LLaMA data, collect SmolLM2 responses, combine and evaluate"""
    ...
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        ...
        for idx, row in tqdm(df_llama.iterrows(), total=len(df_llama), desc="Processing SmolLM2"):  # Updated description
            try:
                ...
                try:
                    responses['direct'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_DIRECT.format(question=q))
                except Exception as e:
                ...
                try:
                    responses['full'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_full))
                except Exception as e:
                ...
                try:
                    responses['corrupt'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_corrupt))
                except Exception as e:
                ...
                try:
                    responses['partial'] = generate_llm_response(SMOLLM2_MODEL, PROMPT_SMOLLM2_WITH_COT.format(question=q, cot=cot_partial))
                except Exception as e:
    ...
    for i in range(min(3, len(df))):
        try:
            ...
            print(f"  SmolLM2 Direct Raw: {smollm2_raw}...")
            print(f"  SmolLM2 Direct Answer: {df.iloc[i]['smollm2_direct_answer']}")
            print()
        except Exception as e:
