...
PROMPT_LLAMA_COT = """You are a logical reasoning assistant. Answer the following question by thinking step-by-step.
...
PROMPT_LLAMA_CORRUPT_COT = """Given the question: "{question}", provide a step-by-step reasoning process that contains:
...
PROMPT_LLAMA_WITH_PARTIAL_COT = """You are a logical reasoning assistant. Below is a partially completed reasoning for a question. Continue from where the reasoning stops and complete the solution to provide the final answer.
...
def remove_final_answer(cot_text):
    ...
    for line in lines:
        ...
        if re.search(r'(Answer:|Therefore.*answer|Final answer|Conclusion)', line, re.IGNORECASE):
...
def extract_yes_no_answer(text):
    """Enhanced Yes/No answer extraction for StrategyQA responses"""
    ...
    end_patterns = [
        r'\b(yes|no)\b\s*\.?\s*$',
        r'therefore.*?\b(yes|no)\b',
        r'conclusion.*?\b(yes|no)\b'
    ]
    ...
    # Method 3: Find all yes/no occurrences and take the last one
    yes_no_matches = list(re.finditer(r'\b(yes|no)\b', text_lower))
    if yes_no_matches:
...
def process_single_question(question_data):
    """Process a single question for StrategyQA with enhanced error handling"""
    idx, q = question_data
    ...
    try:
        ...
        # Generate all LLM responses with individual error handling
        responses = {}
        
        try:
            responses['llama_cot'] = generate_llm_response_sync(
                LLAMA_MODEL, 
                PROMPT_LLAMA_COT.format(question=question_text)
            )
        except Exception as e:
            print(f"Error generating CoT for Q{idx}: {e}")
            responses['llama_cot'] = f"Error: {str(e)}"
        
        try:
            responses['llama_direct'] = generate_llm_response_sync(
                LLAMA_MODEL, 
                PROMPT_DIRECT.format(question=question_text)
            )
        except Exception as e:
            print(f"Error generating direct response for Q{idx}: {e}")
            responses['llama_direct'] = f"Error: {str(e)}"
        
        try:
            responses['corrupted_cot'] = generate_llm_response_sync(
                LLAMA_MODEL, 
                PROMPT_LLAMA_CORRUPT_COT.format(question=question_text)
            )
        except Exception as e:
            print(f"Error generating corrupted CoT for Q{idx}: {e}")
            responses['corrupted_cot'] = f"Error: {str(e)}"
        
        # Check for API errors in main responses
        if any("Error:" in resp for resp in responses.values()):
            error_responses = [k for k, v in responses.items() if "Error:" in v]
            print(f"API Errors for Q{idx} in: {error_responses}")
            # Continue processing if at least one response is valid
            if all("Error:" in resp for resp in responses.values()):
                return None
        
        # Process CoTs
        llama_cot_steps_only = remove_final_answer(responses['llama_cot'])
        corrupted_cot_steps_only = remove_final_answer(responses['corrupted_cot'])
        partial_cot_steps = get_partial_cot(responses['llama_cot'], fraction=0.5)
        
        # Generate partial CoT response
        try:
            llama_partial_cot_response = generate_llm_response_sync(
                LLAMA_MODEL, 
                PROMPT_LLAMA_WITH_PARTIAL_COT.format(
                    question=question_text, 
                    partial_cot=partial_cot_steps
                )
            )
        except Exception as e:
            print(f"Error generating partial CoT response for Q{idx}: {e}")
            llama_partial_cot_response = f"Error: {str(e)}"
        ...
        return {
            ...
            'llama_direct_raw': responses['llama_direct'],
            'llama_cot_raw': responses['llama_cot'],
            'llama_corrupted_cot_raw': responses['corrupted_cot'],
...
def validate_response_format(response, question_type=""):
    """Enhanced validation that response contains Yes/No answer."""
    if not response or "Error:" in response:
        return False
    
    # Use the enhanced extraction function
    extracted_answer = extract_yes_no_answer(response)
    return extracted_answer is not None

def save_results_chunk(results, chunk_num):
    """Save results chunk to CSV efficiently with better error handling"""
    if not results:
...
def main():
    """High-Speed LLaMA Data Collection for StrategyQA Dataset"""
    print("=== 🚀 LLaMA Data Collection Script for StrategyQA Dataset ===")
        ...
        for batch_idx, batch in enumerate(tqdm(batches, desc="Processing Batches")):
            ...
            for future in concurrent.futures.as_completed(future_to_question, timeout=REQUEST_TIMEOUT * 3):
                try:
                    ...
                    print(f"⏰ Timeout collecting result for batch {batch_idx}")
                    format_validation_stats['total_failed'] += 1
                except Exception as e:
    ...
    try:
        if os.path.exists(LLAMA_RESULTS_CSV_PATH):
            ...
            for i in range(min(3, len(df_final))):
                cot_answer = extract_yes_no_answer(df_final.iloc[i].get('llama_cot_raw', ''))
                direct_answer = extract_yes_no_answer(df_final.iloc[i].get('llama_direct_raw', ''))
                print(f"Row {i}: CoT='{cot_answer}', Direct='{direct_answer}', Original='{df_final.iloc[i].get('original_answer_raw', '')}'")
        else:
