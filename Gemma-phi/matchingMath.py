...
df = pd.read_csv('gemma3_phi_math_final_results.csv')
...
df['phi_answer'] = df['phi_with_cot_raw'].apply(lambda x: extract_clean_text(parse_json_answer(x)))

# Calculate match percentage
valid_df = df[(df['gemma3_answer'] != "") & (df['phi_answer'] != "")]
match_rate = (valid_df['gemma3_answer'].str.lower() == valid_df['phi_answer'].str.lower()).mean()
