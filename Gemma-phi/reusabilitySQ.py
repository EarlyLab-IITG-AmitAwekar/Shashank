...
GEMMA3_RESULTS_CSV_PATH = '../gemma3_strategyqa_dataset_results.csv'  # Changed from llama to gemma3
FINAL_RESULTS_CSV_PATH = 'gemma3_phi_strategyqa_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'gemma3_phi_strategyqa_accuracy_summary.csv'  # Updated filename
CHUNK_SAVE_SIZE = 20
...
def main():
    """Load Gemma3 data, collect Phi responses, combine and evaluate"""  # Updated description
    ...
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        ...
        if not os.path.exists(GEMMA3_RESULTS_CSV_PATH):
            print(f"Error: Gemma3 results file '{GEMMA3_RESULTS_CSV_PATH}' not found.")  # Updated error message
            print("Please run the Gemma3 data collection script first.")  # Updated instruction
            return
        ...
        # Load and process
        try:
            df_gemma3 = pd.read_csv(GEMMA3_RESULTS_CSV_PATH)  # Changed from df_llama to df_gemma3
            print(f"Loaded {len(df_gemma3)} rows from Gemma3 results")  # Updated message
        except Exception as e:
            print(f"Error loading Gemma3 results: {e}")  # Updated error message
            return
        
        combined = []
        failed_requests = 0
        
        for idx, row in tqdm(df_gemma3.iterrows(), total=len(df_gemma3), desc="Processing Phi"):  # Changed df_llama to df_gemma3
            try:
                q = safe_get_column_value(row, 'question')
                cot_full = safe_get_column_value(row, 'gemma3_cot_steps_only')  # Changed from llama to gemma3
                ...
                combined.append({
                    ...
                    'gemma3_direct_raw': safe_get_column_value(row, 'gemma3_direct_raw'),  # Changed from llama to gemma3
                    ...
                    'gemma3_cot_steps_only': cot_full,  # Changed from llama to gemma3
                    'corrupted_cot_steps_only': cot_corrupt,
                    'partial_cot_steps': cot_partial,
                    'gemma3_partial_cot_response': safe_get_column_value(row, 'gemma3_partial_cot_response'),  # Changed from llama to gemma3
                ...
                # Periodic save
                if (idx + 1) % CHUNK_SAVE_SIZE == 0 or (idx + 1) == len(df_gemma3):  # Changed df_llama to df_gemma3
                    try:
    ...
    try:
        ...
        df['gemma3_direct_answer'] = df['gemma3_direct_raw'].apply(extract_llm_boolean_answer)  # Changed from llama to gemma3
    ...
    settings = {
        "Gemma3 Direct vs. Original": 'gemma3_direct_answer',  # Changed from LLaMA to Gemma3
