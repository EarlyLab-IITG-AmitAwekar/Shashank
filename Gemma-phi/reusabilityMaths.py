...
FINAL_RESULTS_CSV_PATH = 'gemma3_phi_math_final_results.csv'
SUMMARY_CSV_PATH = 'gemma3_phi_math_accuracy_summary.csv'
CHUNK_SAVE_SIZE = 20
...
PROMPT_PHI_WITH_COT = """You must ONLY use the provided reasoning below to answer the question. 
...
def remove_final_answer_from_cot(cot_text):
    ...
    to ensure only reasoning steps are passed to Phi
    """
...
def process_single_row(args):
    ...
    idx, row_data, phi_model, prompt_direct, prompt_cot = args
    
    try:
        q, cot_steps_only, corrupted_steps_only, partial_steps_only = row_data
        
        # Generate Phi responses
        phi_direct = generate_llm_response(phi_model, prompt_direct.format(question=q))
        phi_with_cot = generate_llm_response(phi_model, prompt_cot.format(question=q, cot=cot_steps_only))
        phi_with_corr = generate_llm_response(phi_model, prompt_cot.format(question=q, cot=corrupted_steps_only))
        phi_with_part = generate_llm_response(phi_model, prompt_cot.format(question=q, cot=partial_steps_only))
        
        return idx, {
            'phi_direct_raw': phi_direct,
            'phi_with_cot_raw': phi_with_cot,
            'phi_with_corrupted_cot_raw': phi_with_corr,
            'phi_with_partial_cot_raw': phi_with_part
        }
    except Exception as e:
        return idx, {
            'phi_direct_raw': f"Error: {e}",
            'phi_with_cot_raw': f"Error: {e}",
            'phi_with_corrupted_cot_raw': f"Error: {e}",
            'phi_with_partial_cot_raw': f"Error: {e}"
        }
...
def parse_json_answer(raw: str):
    ...
    # Strategy 4: Look for "Answer:" pattern (common in Phi responses)
    answer_patterns = [
        r'Answer:\s*(.+?)(?:\n|$)',
        r'The answer is:\s*(.+?)(?:\n|$)',
...
def main():
    print("=== Enhanced Phi Processing with Parallel Execution (Gemma3 Dataset) ===")
    ...
    if not check_if_processing_needed(FINAL_RESULTS_CSV_PATH):
        ...
        for chunk_start in range(0, len(processed_data), chunk_size):
            ...
            for item in chunk_data:
                ...
                args_list.append((item['idx'], row_data, PHI_MODEL, PROMPT_DIRECT, PROMPT_PHI_WITH_COT))
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                results = list(tqdm(
                    ...
                    total=len(args_list),
                    desc=f"Processing Phi responses"
                ))
            
            # Combine results with original data
            for (idx, phi_results), item in zip(results, chunk_data):
                row = item['row']
                combined.append({
    ...
    try:
        ...
        # Extract Phi answers using enhanced JSON/pattern parsing
        print("Parsing Phi answers with enhanced extraction...")
        df_combined['phi_direct_text'] = df_combined['phi_direct_raw'].apply(parse_json_answer)
        df_combined['phi_with_cot_text'] = df_combined['phi_with_cot_raw'].apply(parse_json_answer)
        df_combined['phi_with_corrupted_cot_text'] = df_combined['phi_with_corrupted_cot_raw'].apply(parse_json_answer)
        df_combined['phi_with_partial_cot_text'] = df_combined['phi_with_partial_cot_raw'].apply(parse_json_answer)
        ...
        # Phi comparisons
        print("Computing Phi similarities...")
        phi_direct_sim = compute_similarity_optimized(reference_answers, df_combined['phi_direct_text'].tolist())
        phi_cot_sim = compute_similarity_optimized(reference_answers, df_combined['phi_with_cot_text'].tolist())
        phi_corrupted_sim = compute_similarity_optimized(reference_answers, df_combined['phi_with_corrupted_cot_text'].tolist())
        phi_partial_sim = compute_similarity_optimized(reference_answers, df_combined['phi_with_partial_cot_text'].tolist())
        ...
        df_combined['phi_direct_similarity'] = phi_direct_sim
        df_combined['phi_cot_similarity'] = phi_cot_sim
        df_combined['phi_corrupted_cot_similarity'] = phi_corrupted_sim
        df_combined['phi_partial_cot_similarity'] = phi_partial_sim
        ...
        results = {
            ...
            "Phi Direct vs. Original": np.mean(phi_direct_sim),
            "Phi w/ CoT vs. Original": np.mean(phi_cot_sim),
            "Phi w/ Corrupted CoT vs. Original": np.mean(phi_corrupted_sim),
            "Phi w/ Partial CoT vs. Original": np.mean(phi_partial_sim)
        }

        # Display results
        print("\n" + "="*60)
        print("FINAL RESULTS (Phi vs Gemma3 Optimized Processing)")
        print("="*60)
