...
STABLELM_MODEL = 'stablelm2:1.6b'  # Changed from 'smollm2:1.7b'
GEMMA_RESULTS_CSV_PATH = '../gemma3_svamp_results.csv'
FINAL_RESULTS_CSV_PATH = 'gemma_stablelm_svamp_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'gemma_stablelm_svamp_accuracy_summary.csv'  # Updated filename
...
def process_single_question(args):
    ...
    try:
        # Generate StableLM responses
        resp_direct = generate_llm_response(STABLELM_MODEL, PROMPT_DIRECT.format(question=q))
        resp_full = generate_llm_response(STABLELM_MODEL, PROMPT_STABLELM_WITH_COT.format(question=q, cot=cot_full))
        resp_corrupt = generate_llm_response(STABLELM_MODEL, PROMPT_STABLELM_WITH_COT.format(question=q, cot=cot_corrupt))
        resp_partial = generate_llm_response(STABLELM_MODEL, PROMPT_STABLELM_WITH_COT.format(question=q, cot=cot_partial))
        
        return idx, {
            'stablelm_direct_raw': resp_direct,  # Changed from 'smollm_direct_raw'
            'stablelm_with_cot_raw': resp_full,  # Changed from 'smollm_with_cot_raw'
            'stablelm_with_corrupted_cot_raw': resp_corrupt,  # Changed from 'smollm_with_corrupted_cot_raw'
            'stablelm_with_partial_cot_raw': resp_partial,  # Changed from 'smollm_with_partial_cot_raw'
        }
    except Exception as e:
        return idx, {
            'stablelm_direct_raw': f"Error: {str(e)}",
            'stablelm_with_cot_raw': f"Error: {str(e)}",
            'stablelm_with_corrupted_cot_raw': f"Error: {str(e)}",
            'stablelm_with_partial_cot_raw': f"Error: {str(e)}",
        }
...
def safe_get_column_value(row, column_name, default_value=""):
    ...
    try:
        ...
        print(f"Warning: Column '{column_name}' not found, using default value")
        return default_value


def main():
    """Load Gemma data, collect StableLM responses, combine and evaluate"""
    print("=== StableLM Processing and Evaluation Script (SVAMP Dataset) ===")  # Updated print message


    # Verify Gemma results exist
    if not os.path.exists(GEMMA_RESULTS_CSV_PATH):
        print(f"Error: Gemma results file '{GEMMA_RESULTS_CSV_PATH}' not found.")
        print("Please run the Gemma SVAMP data collection script first.")
        return
    ...
    # Load and process
    df_gemma = pd.read_csv(GEMMA_RESULTS_CSV_PATH)
    print(f"Loaded {len(df_gemma)} rows from Gemma SVAMP results")
    
    # Debug: Check available columns
    print("Available columns in CSV:")
    print(df_gemma.columns.tolist())
    
    # Prepare data for parallel processing
    print(f"Preparing data for parallel processing with {MAX_WORKERS} workers...")
    
    processed_data = []
    for idx, row in df_gemma.iterrows():
        q = safe_get_column_value(row, 'question')
        if not q.strip():
            continue
        
        # Updated column names to match your CSV headers
        cot_full = safe_get_column_value(row, 'gemma3_cot_steps_only')  # Changed from 'gemma_cot_steps_only'
    ...
    for batch_start in range(0, len(processed_data), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Processing StableLM responses"):  # Updated description
                try:
                    idx, stablelm_results = future.result()  # Changed variable name
                    ...
                    combined.append({
                        ...
                        'gemma3_cot_steps_only': safe_get_column_value(row, 'gemma3_cot_steps_only'),
                        ...
                        **stablelm_results  # Changed from smollm_results
                    })
                except Exception as e:
    ...
    # Updated column names to match your CSV headers
    df['gemma3_direct_answer'] = df['gemma3_direct_raw'].apply(extract_llm_answer)
    ...
    df['stablelm_direct_answer'] = df['stablelm_direct_raw'].apply(extract_llm_answer)  # Changed from smollm
    df['stablelm_with_cot_answer'] = df['stablelm_with_cot_raw'].apply(extract_llm_answer)  # Changed from smollm
    df['stablelm_with_corrupted_cot_answer'] = df['stablelm_with_corrupted_cot_raw'].apply(extract_llm_answer)  # Changed from smollm
    df['stablelm_with_partial_cot_answer'] = df['stablelm_with_partial_cot_raw'].apply(extract_llm_answer)  # Changed from smollm
    ...
    settings = {
        ...
        "StableLM Direct vs. Original": 'stablelm_direct_answer',  # Changed from SmolLM
        "StableLM with CoT vs. Original": 'stablelm_with_cot_answer',  # Changed from SmolLM
        "StableLM with Corrupted CoT vs. Original": 'stablelm_with_corrupted_cot_answer',  # Changed from SmolLM
        "StableLM with Partial CoT vs. Original": 'stablelm_with_partial_cot_answer',  # Changed from SmolLM
    }
