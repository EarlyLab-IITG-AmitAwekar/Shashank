...
GEMMA3_MODEL = 'stablelm2:1.6b'  # Keeping StableLM2 as the secondary model
GEMMA_RESULTS_CSV_PATH = '../gemma3_gsm8k_results.csv'  # Changed from llama to gemma3
FINAL_RESULTS_CSV_PATH = 'gemma3_stablelm2_gsm8k_final_results.csv'  # Updated filename
SUMMARY_CSV_PATH = 'gemma3_stablelm2_gsm8k_accuracy_summary.csv'  # Updated filename
...
PROMPT_STABLELM2_WITH_COT = """  # Renamed for clarity
...
def main():
    """Load Gemma 3 data, collect StableLM2 responses, combine and evaluate"""  # Updated description
    print("=== Gemma 3 + StableLM2 Processing and Evaluation Script ===")  # Updated print statement
    ...
    # Load and process
    df_gemma = pd.read_csv(GEMMA_RESULTS_CSV_PATH)  # Changed variable name
    combined = []
    for idx, row in tqdm(df_gemma.iterrows(), total=len(df_gemma), desc="Processing StableLM2"):  # Updated description
        q = row['question']
        ...
        combined.append({
            ...
            'stablelm2_direct_raw':           resp_direct,  # Updated column names
        ...
        # Periodic save
        if (idx+1) % CHUNK_SAVE_SIZE == 0 or (idx+1)==len(df_gemma):  # Updated variable name
    ...
    df['stablelm2_direct_answer']      = df['stablelm2_direct_raw'].apply(extract_llm_answer)  # Updated column names
